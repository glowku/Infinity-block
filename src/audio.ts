/**
 * Procedural retro arcade audio synthesizer using the Web Audio API.
 * High-performance, lightweight, zero dependencies.
 */
...
class SoundEngine {
  ...
  constructor() {
    try {
      const saved = localStorage.getItem('dem_block_muted');
      if (saved !== null) {
  ...
  public toggleMute(): boolean {
    ...
    try {
      localStorage.setItem('dem_block_muted', String(this.isMuted));
    } catch {
      // ignore
    }
    return this.isMuted;
  ...
  public shoot(isSuper: boolean = false, isBounce: boolean = false, multiCount: number = 1) {
    ...
    osc.type = isSuper ? 'sawtooth' : isBounce ? 'triangle' : 'square';

    const baseFreq = isSuper ? 780 : isBounce ? 920 : 640 + multiCount * 40;
    osc.frequency.setValueAtTime(baseFreq, now);
    osc.frequency.exponentialRampToValueAtTime(80, now + 0.09);

    gain.gain.setValueAtTime(isSuper ? 0.22 : 0.14, now);
  ...
  public explode() {
    ...
    filter.frequency.exponentialRampToValueAtTime(40, now + 0.32);
